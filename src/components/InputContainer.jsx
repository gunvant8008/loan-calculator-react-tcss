          ...
          {dataPoints.map((point, index) => {
            return (
              <li key={index} className="flex justify-center relative">
                <span className="absolute text-gray-500">{point}</span>
              </li>
            )
          })}
        </ul>
...
const InputContainer = ({ ranges, setRanges }) => {
  ...
  return (
    <div className=" w-[80%] flex flex-col items-center p-8 shadow-lg gap-8 rounded-2xl bg-gray-300 backdrop-blur-[2px]">
      <Input
        title={"Loan Amount $"}
